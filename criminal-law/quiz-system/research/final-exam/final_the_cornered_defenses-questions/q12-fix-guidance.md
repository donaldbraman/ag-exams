...
<!-- audit: MUST FIX
<check 1>: fail (Option (a) incorrectly assumes Leo has a qualifying mental disease like schizophrenia, which is never established in the stem)
<check 2>: pass (The distractors are doctrinally false, though a student might be tempted to guess one if they assume Leo only has APD)
<check 3>: pass (Explanation accurately reflects the co-occurring causation doctrine)
<check 4>: fail (The stem completely fails to mention that Leo has a co-occurring qualifying mental disease; without this fact, a student must assume Leo only has APD, meaning the defense *would* fail)
<check 5>: pass (The jurisdictional rule on APD is expressly stipulated)
<check 6>: pass (No excluded topics)
<check 7>: pass (Doctrine matches `co-occurring-causation` and `personality-disorder-exclusion` in Ch 23)
Recommended fix: Add facts to the stem establishing a co-occurring diagnosis. E.g., "Leo suffers from severe schizophrenia but also has a diagnosis of Antisocial Personality Disorder (APD). Experts testify that both conditions contributed to his cognitive incapacity during the offense. Assume that the jurisdiction..."
-->
