# Fix Guidance for q14

The QA pipeline flagged this question. Rewrite `q14.md` addressing each numbered issue below. Do NOT delete this guidance file — the pipeline handles it.

## Issue 1 — audit

**Q14.** How would Silas's decision to unholster his weapon instead of driving away be evaluated in a traditional duty-to-retreat jurisdiction compared to a Stand Your Ground jurisdiction?

(a) Both jurisdictions would require him to drive away because the duty to retreat universally applies when the defendant is seated in a mobile vehicle.
(b) A duty-to-retreat jurisdiction would bar his self-defense claim because he had a safe path, while a Stand Your Ground jurisdiction would permit his defensive force. <!-- correct -->
(c) A duty-to-retreat jurisdiction would allow his claim because of the castle doctrine, while a Stand Your Ground jurisdiction would strictly deny it.
(d) Both jurisdictions would allow his claim because the duty to retreat only applies to physical flight on foot, not to fleeing inside a motor vehicle.
(e) A Stand Your Ground jurisdiction would bar his claim because Trey was unarmed, whereas a duty-to-retreat jurisdiction exclusively examines the defendant's subjective fear.

**Answer:** (b)

**Explanation:** In a traditional duty-to-retreat jurisdiction, a defendant must retreat before using deadly force if a safe path is available (such as Silas's clear, unobstructed path to drive away). A Stand Your Ground jurisdiction explicitly eliminates this duty, allowing the defendant to meet force with force in any place they have a legal right to be. (a) is wrong because Stand Your Ground jurisdictions completely eliminate the duty to retreat, regardless of vehicle mobility. (c) is wrong because the castle doctrine typically does not extend to public gas stations or cars in the same way as a home. (d) is wrong because the traditional duty to retreat applies to any safe means of escape, including vehicles. (e) is wrong because Stand Your Ground laws do not hinge on whether the victim was actually armed.

**Tags:** chapters: [22], topics: [self-defense, duty to retreat, stand your ground], difficulty: medium, cognitive: application

**Grounding:** Chapter 22 (Self-Defense), duty-to-retreat-general and stand-your-ground-statute refinements.

<!-- audit: MUST FIX
check 1: pass (the doctrinal distinction is handled correctly, assuming facts)
check 2: pass (distractors are well-calibrated to common misconceptions)
check 3: pass (the explanation correctly applies the rules for SYG vs. duty-to-retreat)
check 4: MUST FIX (The question is completely missing its underlying fact pattern. The options and explanation reference facts not in the stem—e.g., Silas being at a public gas station, Trey being the victim, Silas having an unobstructed path to drive away, and Trey being unarmed. A student cannot evaluate the application without these facts.)
check 5: pass (the question cleanly juxtaposes the two rules without requiring rote memorization of specific states)
check 6: pass (no excluded topics present)
check 7: pass (tags properly ground to `duty-to-retreat-general`, `castle-doctrine-scope`, and `stand-your-ground-statute`)
Recommended fix: Prepend the full fact pattern describing the gas station encounter between Silas and Trey, establishing Trey's threatening behavior, Silas's safe path of retreat in his vehicle, and his decision to unholster his weapon.
-->

## Issue 2 — edge-case

**Q14.** How would Silas's decision to unholster his weapon instead of driving away be evaluated in a traditional duty-to-retreat jurisdiction compared to a Stand Your Ground jurisdiction?

(a) Both jurisdictions would require him to drive away because the duty to retreat universally applies when the defendant is seated in a mobile vehicle.
(b) A duty-to-retreat jurisdiction would bar his self-defense claim because he had a safe path, while a Stand Your Ground jurisdiction would permit his defensive force. <!-- correct -->
(c) A duty-to-retreat jurisdiction would allow his claim because of the castle doctrine, while a Stand Your Ground jurisdiction would strictly deny it.
(d) Both jurisdictions would allow his claim because the duty to retreat only applies to physical flight on foot, not to fleeing inside a motor vehicle.
(e) A Stand Your Ground jurisdiction would bar his claim because Trey was unarmed, whereas a duty-to-retreat jurisdiction exclusively examines the defendant's subjective fear.

**Answer:** (b)

**Explanation:** In a traditional duty-to-retreat jurisdiction, a defendant must retreat before using deadly force if a safe path is available (such as Silas's clear, unobstructed path to drive away). A Stand Your Ground jurisdiction explicitly eliminates this duty, allowing the defendant to meet force with force in any place they have a legal right to be. (a) is wrong because Stand Your Ground jurisdictions completely eliminate the duty to retreat, regardless of vehicle mobility. (c) is wrong because the castle doctrine typically does not extend to public gas stations or cars in the same way as a home. (d) is wrong because the traditional duty to retreat applies to any safe means of escape, including vehicles. (e) is wrong because Stand Your Ground laws do not hinge on whether the victim was actually armed.

**Tags:** chapters: [22], topics: [self-defense, duty to retreat, stand your ground], difficulty: medium, cognitive: application

**Grounding:** Chapter 22 (Self-Defense), duty-to-retreat-general and stand-your-ground-statute refinements.

<!-- edge-case-audit: MUST FIX
1. Fact Pattern Booby Traps: The facts state Trey was unarmed and merely walking to a convenience store. Stand Your Ground (SYG) laws remove the duty to retreat, but they do NOT waive the core requirements of self-defense (i.e., objective reasonableness and imminence). Stating that a SYG jurisdiction would definitively "permit his defensive force" is legally inaccurate here, as his claim may still fail on those other grounds.
2. Cross-Doctrine Clashes: pass
3. Cross-Question Spoilers: Q13 and Q15 test objective reasonableness and imperfect self-defense for this exact encounter. Option (b) spoils those questions by inadvertently declaring that Silas's use of force is permitted/justified under SYG.
Recommended fix: Change Option (b) to: "A duty-to-retreat jurisdiction would bar his self-defense claim because he had a safe path, while a Stand Your Ground jurisdiction would not bar the claim solely for his failure to retreat." Update the explanation to note: "A Stand Your Ground jurisdiction explicitly eliminates this duty, meaning his claim would not fail automatically for choosing to stay (though it must still satisfy objective reasonableness)."
-->
