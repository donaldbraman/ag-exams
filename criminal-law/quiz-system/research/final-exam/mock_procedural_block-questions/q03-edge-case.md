...
1. Fact Pattern Booby Traps: pass. The use of "only key" is not a booby trap, but rather the precise factual hook needed to establish exclusive control, properly supporting the legal conclusion of constructive possession.
2. Cross-Doctrine Clashes: pass. The intent to destroy the drugs (as mentioned in option e) constitutes an exercise of dominion and control, meaning it does not clash with or negate the legal requirements for simple possession. 
3. Cross-Question Spoilers: pass. While Q11 establishes the drugs belonged to a rival syndicate, legal ownership is not required for constructive possession—only the power and intent to exercise dominion and control (which Silas demonstrated by possessing the only key). Furthermore, DA Miller deleted this evidence, so the trial evidence remains entirely sufficient.
Recommended fix: n/a
-->