...
<!-- audit: SHOULD FIX
check 1: pass
check 2: finding: A sharp student might argue that a 20% salary premium permits a jury to *infer* purpose, but does not conclusively "establish" it as a matter of law (which would imply a mandatory presumption). Option (d) could be slightly softened to reflect this evidentiary inference.
check 3: pass
check 4: finding: The "twenty percent salary premium" is not mentioned in the question stem. Assuming this fact is clearly established in the overarching shared fact pattern, this passes. If not, it improperly requires facts beyond the stem.
check 5: pass
check 6: pass
check 7: pass
check 8: pass
Recommended fix: In option (d), change "establishes a purposeful desire" to "permits an inference of a purposeful desire" or "provides a basis to find a purposeful desire". Verify the 20% premium is explicitly detailed in the shared fact pattern.
-->